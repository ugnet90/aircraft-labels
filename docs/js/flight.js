...
function analyzeFlight(models, flight){
  ...
  const sameAirlineFn = (m) => {
    const fAir = normalize(flight.airline_row) || normalize(flight.airline) || normalize(flight.logo_id);
...
async function main(){
  ...
  try{
    ...
    const airline = asText(f.airline_row) || asText(f.airline) || asText(f.logo_id);
